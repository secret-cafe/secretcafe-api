...
export class MenuService {
  ...
  async create(data: CreateMenuDto, file?: any, createdById?: number) {
    try {
      ...
      const categoryInternalId =
        await this.resolveCategoryIdOrThrow(categoryId);
  ...
  async findAll(query?: QueryMenuDto) {
    ...
    if (query?.categoryId) {
      categoryInternalId = await this.resolveCategoryIdOrThrow(
        query.categoryId,
      );
    }
  ...
  async findByCategory(categoryId: string) {
    ...
    return {
      ...
      data: {
        ...rest,
        id: cid,
        menuItems: categoryMenu.menuItems.map((item) =>
          this.transformToLegacyFormat(item),
        ),
      },
    };
  }
      ...
      await this.prisma.menuItem.update({
        ...
        data: {
          ...
          ...(categoryId !== undefined && {
            categoryId: await this.resolveCategoryIdOrThrow(categoryId),
          }),
          ...(submenu !== undefined && {
            menuSubMenus: {
   ...
   */
  private async prepareSubMenuCreates(
    submenu: any[],
    createdById?: number,
  ): Promise<any[]> {
    const result: any[] = [];
