...
export class OrderItemService {
  ...
  async prepareNewItems(
    ...
    const allSubIds = [
      ...new Set(
        items.flatMap((i) =>
          (i.orderSubMenuItems || []).map((s) => s.subMenuItemId),
        ),
      ),
    ];
  ...
  private async updateExistingItem(
    ...
    const menuMap = await this.validation.resolveMenuItems([
      incoming.menuItemId,
    ]);
    ...
    const activeSubs = existing.orderSubMenuItem.filter(
      (s: any) => !s.isCancelled,
    );
    ...
    const allNeededSubIds = (incoming.orderSubMenuItems || []).map(
      (s) => s.subMenuItemId,
    );
    ...
    for (const incomingSub of incoming.orderSubMenuItems || []) {
      const match = activeSubs.find(
        (s: any) => s.subMenuItem?.subMenuId === incomingSub.subMenuItemId,
      );
      if (match) {
  ...
  private async createNewItem(
    ...
    const allSubIds = (incoming.orderSubMenuItems || []).map(
      (s) => s.subMenuItemId,
    );
    const subMap = await this.validation.resolveSubMenuItems(allSubIds);
  ...
  private buildSubMenuData(
    ...
    return subItems.map((sub) => {
      const subMenuItem = subMap.get(sub.subMenuItemId);
      const qty = sub.quantity ?? 1;
      const total = subMenuItem
        ? subMenuItem.price.mul(qty)
        : new Prisma.Decimal(0);
