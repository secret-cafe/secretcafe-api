...
export class OrderService {
  ...
  public async updateOrderStatus(
    ...
    if (dto.isItemsUpdate) {
      ...
      await this.prisma.$transaction(async (tx) => {
        const recalculatedSubtotal = await this.itemService.recalculateSubtotal(
          tx,
          order.id,
        );
        await tx.order.update({
  ...
  private async syncOrderStatus(orderId: number) {
    ...
    const maxStatus = items.reduce<OrderStatus>(
      (prev, curr) =>
        this.validation.statusPriority[curr.status] >
        this.validation.statusPriority[prev]
          ? curr.status
          : prev,
      OrderStatus.PENDING,
    );
